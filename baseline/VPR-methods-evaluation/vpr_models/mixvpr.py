...
def get_mixvpr(descriptors_dimension):
    ...
    if not os.path.exists(file_path):
        ...
        downloaded = gdown.download(url=url, output=file_path, fuzzy=True, quiet=False)
        if downloaded is None or not os.path.exists(file_path):
            raise RuntimeError(
                f"Failed to download MixVPR checkpoint from {url}. "
                f"Please manually download the file and place it at: {file_path}"
            )
