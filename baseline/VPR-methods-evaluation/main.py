...
def main(args):
    ...
    logger.info(f"Using retrieval metric: {args.metric}")
    ...
    with torch.inference_mode():
        ...
        for images, indices in tqdm(queries_dataloader):
            ...
            query_inference_times.append(batch_elapsed)
            all_descriptors[indices.numpy(), :] = descriptors
    ...
    if args.metric == "dot":
        database_descriptors = normalize_descriptors(database_descriptors)
        queries_descriptors = normalize_descriptors(queries_descriptors)
    ...
    avg_model_time = np.mean(query_inference_times) if query_inference_times else 0.0
    avg_faiss_time = search_elapsed / test_ds.num_queries if test_ds.num_queries > 0 else 0.0
    logger.info(f"Average model inference time per query: {avg_model_time:.6f}s")
    logger.info(f"Average FAISS search time per query: {avg_faiss_time:.6f}s")
    logger.info(f"Average total time per query: {avg_model_time + avg_faiss_time:.6f}s")
